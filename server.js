...
// makes it easier to use get and post requests
const app = express();
...
app.get('/', (request, response) => {
  // responds back by send the index.html file to the client
  response.sendFile(__dirname + '/index.html');
});

// the colon in the url is what is telling express that this is a variable
app.get('/api/:alienName', (request, response) => {
  const alienName = request.params.alienName.toLowerCase();
  if (aliens[alienName]) {
    // if the alien exists, send the alien back to the client
    response.json(aliens[alienName]);
  } else {
    // responds with humans if the alien doesn't exist
